...
from python_speech_features import get_filterbanks,lifter
...
def fbank(signal, samplerate=16000, winlen=0.025, winstep=0.01,
    ...
    fb = centerlized_mfcc(0, 4650, 8000)
    feat = numpy.dot(pspec, fb.T)  # compute the filterbank energies
    feat = numpy.where(feat == 0, numpy.finfo(float).eps, feat)  # if feat is zero, we get problems with log

    return feat, energy

def centerlized_mfcc(lowFreq=0,middleFreq=4650,highFreq=8000):
    ...
    fb2 = get_filterbanks(nfilt=11,nfft=512,samplerate=16000,lowfreq=0,highfreq=3500)
    fb = numpy.zeros(257)
    for filter in fb1:
